...
clean:
	-rm -rf \
		$(PUBLIC_DIR) \
		volaris/data/_libs/*.cpp \
		volaris/data/_libs/*.so \
		mlruns \
		public \
		build \
		.coverage \
...
prerequisite:
	@if [ -n "$(SO_FILES)" ]; then \
		...
		python -c "from setuptools import setup, Extension; from Cython.Build import cythonize; import numpy; extensions = [Extension('volaris.data._libs.rolling', ['volaris/data/_libs/rolling.pyx'], language='c++', include_dirs=[numpy.get_include()]), Extension('volaris.data._libs.expanding', ['volaris/data/_libs/expanding.pyx'], language='c++', include_dirs=[numpy.get_include()])]; setup(ext_modules=cythonize(extensions, language_level='3'), script_args=['build_ext', '--inplace'])"; \
	fi
...
black:
	black . -l 120 --check --diff --exclude volaris/_version.py
...
# 			volaris/backtest/executor.py
# 			volaris/data/ops.py
# 			volaris/utils/__init__.py
...
flake8:
	flake8 --ignore=E501,F541,E266,E402,W503,E731,E203 --per-file-ignores="__init__.py:F401,F403" volaris

# Check code with mypy.
# https://github.com/python/mypy/issues/10600
mypy:
	mypy volaris --install-types --non-interactive
	mypy volaris --verbose
