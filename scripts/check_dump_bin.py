...
class CheckBin:
    ...
    def __init__(
        ...
        check_fields : str, optional
            check fields, by default None, check volaris_dir/features/<first_dir>/*.<freq>.bin
        freq : str, optional
        ...
        """
        self.volaris_dir = Path(volaris_dir).expanduser()
        bin_path_list = list(self.volaris_dir.joinpath("features").iterdir())
        self.volaris_symbols = sorted(map(lambda x: x.name.lower(), bin_path_list))
        volaris.init(
            provider_uri=str(self.volaris_dir.resolve()),
            mount_path=str(self.volaris_dir.resolve()),
            auto_mount=False,
            redis_port=-1,
        )
        ...
        self.volaris_fields = list(map(lambda x: f"${x}", self.check_fields))
    ...
    def _compare(self, file_path: Path):
        ...
        if symbol.lower() not in self.volaris_symbols:
            return self.NOT_IN_FEATURES
        # volaris data
        volaris_df = D.features([symbol], self.volaris_fields, freq=self.freq)
        volaris_df.rename(columns={_c: _c.strip("$") for _c in volaris_df.columns}, inplace=True)
        # csv data
        ...
        origin_df.index.names = volaris_df.index.names
        origin_df = origin_df.reindex(volaris_df.index)
        try:
