...
from volaris.utils import fname_to_code, code_to_fname
...
class DumpDataBase:
    ...
    def __init__(
        ...
        volaris_dir: str
            volaris(dump) data director
        backup_dir: str, default None
            if backup_dir is not None, backup volaris_dir to backup_dir
        ...
        if backup_dir is not None:
            self._backup_volaris_dir(Path(backup_dir).expanduser())
        ...
        self._calendars_dir = self.volaris_dir.joinpath(self.CALENDARS_DIR_NAME)
        self._features_dir = self.volaris_dir.joinpath(self.FEATURES_DIR_NAME)
        self._instruments_dir = self.volaris_dir.joinpath(self.INSTRUMENTS_DIR_NAME)
    ...
    def _backup_volaris_dir(self, target_dir: Path):
        shutil.copytree(str(self.volaris_dir.resolve()), str(target_dir.resolve()))
