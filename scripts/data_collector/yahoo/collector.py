...
from volaris.utils import code_to_fname, fname_to_code, exists_volaris_data
from volaris.constant import REG_CN as REGION_CN
...
class YahooNormalize1dExtend(YahooNormalize1d):
    def __init__(
        self, old_volaris_data_dir: [str, Path], date_field_name: str = "date", symbol_field_name: str = "symbol", **kwargs
    ):
        """

        Parameters
        ----------
        old_volaris_data_dir: str, Path
            the volaris data to be updated for yahoo, usually from: https://github.com//volaris/tree/main/scripts#download-cn-data
        date_field_name: str
        ...
        self.old_volaris_data = self._get_old_data(old_volaris_data_dir)

    def _get_old_data(self, volaris_data_dir: [str, Path]):
        volaris_data_dir = str(Path(volaris_data_dir).expanduser().resolve())
        volaris.init(provider_uri=volaris_data_dir, expression_cache=None, dataset_cache=None)
    ...
    def normalize(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        old_symbol_list = self.old_volaris_data.index.get_level_values("instrument").unique().to_list()
        if str(symbol_name).upper() not in old_symbol_list:
            return df.reset_index()
        old_df = self.old_volaris_data.loc[str(symbol_name).upper()]
...
class YahooNormalize1min(YahooNormalize, ABC):
    ...
    def __init__(
        self, volaris_data_1d_dir: [str, Path], date_field_name: str = "date", symbol_field_name: str = "symbol", **kwargs
    ):
        """

        Parameters
        ----------
        volaris_data_1d_dir: str, Path
            the volaris data to be updated for yahoo, usually from: Normalised to 1min using local 1d data
        date_field_name: str
            date field name, default is date
        symbol_field_name: str
            symbol field name, default is symbol
        ...
        volaris.init(provider_uri=volaris_data_1d_dir)
...
class Run(BaseRun):
    ...
    def download_data(
        ...
        ---------
            # get daily data
            $ python collector.py download_data --source_dir ~/.volaris/stock_data/source --region CN --start 2020-11-01 --end 2020-11-10 --delay 0.1 --interval 1d
            # get 1m data
            $ python collector.py download_data --source_dir ~/.volaris/stock_data/source --region CN --start 2020-11-01 --end 2020-11-10 --delay 0.1 --interval 1m
        """
        ...
        volaris_data_1d_dir: str
            if interval==1min, volaris_data_1d_dir cannot be None, normalize 1min needs to use 1d data;
                ...
                volaris_data_1d can be obtained like this:
                    ...
                    $ python scripts/data_collector/yahoo/collector.py update_data_to_bin --volaris_data_1d_dir <volaris_data_1d_dir> --trading_date 2021-06-01
                or:
                    download 1d data, reference: https://github.com//volaris/tree/main/scripts/data_collector/yahoo#1d-from-yahoo

        Examples
        ---------
            $ python collector.py normalize_data --source_dir ~/.volaris/stock_data/source --normalize_dir ~/.volaris/stock_data/normalize --region cn --interval 1d
            $ python collector.py normalize_data --volaris_data_1d_dir ~/.volaris/volaris_data/cn_data --source_dir ~/.volaris/stock_data/source_cn_1min --normalize_dir ~/.volaris/stock_data/normalize_cn_1min --region CN --interval 1min
        """
        if self.interval.lower() == "1min":
            if volaris_data_1d_dir is None or not Path(volaris_data_1d_dir).expanduser().exists():
                raise ValueError(
                    "If normalize 1min, the volaris_data_1d_dir parameter must be set: --volaris_data_1d_dir <user volaris 1d data >, Reference: https://github.com//volaris/tree/main/scripts/data_collector/yahoo#automatic-update-of-daily-frequency-datafrom-yahoo-finance"
                )
        super(Run, self).normalize_data(
            date_field_name, symbol_field_name, end_date=end_date, volaris_data_1d_dir=volaris_data_1d_dir
        )

    def normalize_data_1d_extend(
        self, old_volaris_data_dir, date_field_name: str = "date", symbol_field_name: str = "symbol"
    ):
        """normalize data extend; extending yahoo volaris data(from: https://github.com//volaris/tree/main/scripts#download-cn-data)

        Notes
        -----
            Steps to extend yahoo volaris data:

                1. download volaris data: https://github.com//volaris/tree/main/scripts#download-cn-data; save to <dir1>

                2. collector source data: https://github.com//volaris/tree/main/scripts/data_collector/yahoo#collector-data; save to <dir2>

                3. normalize new source data(from step 2): python scripts/data_collector/yahoo/collector.py normalize_data_1d_extend --old_volaris_dir <dir1> --source_dir <dir2> --normalize_dir <dir3> --region CN --interval 1d

                4. dump data: python scripts/dump_bin.py dump_update --data_path <dir3> --volaris_dir <dir1> --freq day --date_field_name date --symbol_field_name symbol --exclude_fields symbol,date

                5. update instrument(eg. csi300): python python scripts/data_collector/cn_index/collector.py --index_name CSI300 --volaris_dir <dir1> --method parse_instruments

        Parameters
        ----------
        old_volaris_data_dir: str
            the volaris data to be updated for yahoo, usually from: https://github.com//volaris/tree/main/scripts#download-cn-data
        date_field_name: str
            date field name, default date
        symbol_field_name: str
        ...
        Examples
        ---------
            $ python collector.py normalize_data_1d_extend --old_volaris_dir ~/.volaris/volaris_data/cn_data --source_dir ~/.volaris/stock_data/source --normalize_dir ~/.volaris/stock_data/normalize --region CN --interval 1d
        """
        ...
        yc = Normalize(
            ...
            old_volaris_data_dir=old_volaris_data_dir,
        )
        yc.normalize()
    ...
    def download_today_data(
        ...
        ---------
            # get daily data
            $ python collector.py download_today_data --source_dir ~/.volaris/stock_data/source --region CN --delay 0.1 --interval 1d
            # get 1m data
            $ python collector.py download_today_data --source_dir ~/.volaris/stock_data/source --region CN --delay 0.1 --interval 1m
        """
    ...
    def update_data_to_bin(
        ...
        Notes
        -----
            If the data in volaris_data_dir is incomplete, np.nan will be populated to trading_date for the previous trading day

        Examples
        -------
            $ python collector.py update_data_to_bin --volaris_data_1d_dir <user data dir> --trading_date <start date> --end_date <end date>
        """

        if self.interval.lower() != "1d":
            logger.warning(f"currently supports 1d data updates: --interval 1d")

        # download volaris 1d data
        volaris_data_1d_dir = str(Path(volaris_data_1d_dir).expanduser().resolve())
        if not exists_volaris_data(volaris_data_1d_dir):
            GetData().volaris_data(
                target_dir=volaris_data_1d_dir, interval=self.interval, region=self.region, exists_skip=exists_skip
            )

        # start/end date
        calendar_df = pd.read_csv(Path(volaris_data_1d_dir).joinpath("calendars/day.txt"))
        ...
        # normalize data
        self.normalize_data_1d_extend(volaris_data_1d_dir)

        # dump bin
        _dump = DumpDataUpdate(
            data_path=self.normalize_dir,
            volaris_dir=volaris_data_1d_dir,
        ...
        for _index in index_list:
            get_instruments(str(volaris_data_1d_dir), _index, market_index=f"{_region}_index")
