...
def get_us_stock_symbols(volaris_data_path: [str, Path] = None) -> list:
    ...
    if _US_SYMBOLS is None:
        ...
        if volaris_data_path is not None:
            for _index in ["nasdaq100", "sp500"]:
                ins_df = pd.read_csv(
                    Path(volaris_data_path).joinpath(f"instruments/{_index}.txt"),
                    sep="\t",
...
def get_in_stock_symbols(volaris_data_path: [str, Path] = None) -> list:
...
def get_br_stock_symbols(volaris_data_path: [str, Path] = None) -> list:
...
def get_en_fund_symbols(volaris_data_path: [str, Path] = None) -> list:
    """get en fund symbols
...
def get_instruments(
    ...
    ----------
    volaris_dir: str
        volaris data dir, default "Path(__file__).parent/volaris_data"
    index_name: str
    ...
    -------
        # parse instruments
        $ python collector.py --index_name CSI300 --volaris_dir ~/.volaris/volaris_data/cn_data --method parse_instruments

        # parse new companies
        $ python collector.py --index_name CSI300 --volaris_dir ~/.volaris/volaris_data/cn_data --method save_new_companies
    ...
    obj = getattr(_cur_module, f"{index_name.upper()}Index")(
        volaris_dir=volaris_dir, index_name=index_name, freq=freq, request_retry=request_retry, retry_sleep=retry_sleep
    )
    getattr(obj, method)()
