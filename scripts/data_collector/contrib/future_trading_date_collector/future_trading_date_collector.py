...
def read_calendar_from_volaris(volaris_dir: Path) -> pd.DataFrame:
    calendar_path = volaris_dir.joinpath("calendars").joinpath("day.txt")
    if not calendar_path.exists():
        return pd.DataFrame()
    return pd.read_csv(calendar_path, header=None)


def write_calendar_to_volaris(volaris_dir: Path, date_list: List[str], freq: str = "day"):
    calendar_path = str(volaris_dir.joinpath("calendars").joinpath(f"{freq}_future.txt"))

    np.savetxt(calendar_path, date_list, fmt="%s", encoding="utf-8")
    logger.info(f"write future calendars success: {calendar_path}")


def generate_volaris_calendar(date_list: List[str], freq: str) -> List[str]:
    print(freq)
    if freq == "day":
        return date_list
    elif freq == "1min":
...
def future_calendar_collector(volaris_dir: [str, Path], freq: str = "day"):
    """get future calendar

    Parameters
    ----------
    volaris_dir: str or Path
        volaris data directory
    freq: str
        value from ["day", "1min"], by default day
    """
    volaris_dir = Path(volaris_dir).expanduser().resolve()
    if not volaris_dir.exists():
        raise FileNotFoundError(str(volaris_dir))
    ...
    if lg.error_code != "0":
        ...
        return
    # read daily calendar
    daily_calendar = read_calendar_from_volaris(volaris_dir)
    ...
    date_list = generate_volaris_calendar(data_list, freq=freq)
    date_list = sorted(set(daily_calendar.loc[:, 0].values.tolist() + date_list))
    write_calendar_to_volaris(volaris_dir, date_list, freq=freq)
    bs.logout()
