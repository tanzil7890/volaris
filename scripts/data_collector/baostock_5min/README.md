...
### Get Volaris data(`bin file`)
  ...
  - parameters:
    - `target_dir`: save dir, by default *~/.volaris/volaris_data/cn_data_5min*
  ...
  - examples:
    ```bash
    # hs300 5min
    python scripts/get_data.py volaris_data --target_dir ~/.volaris/volaris_data/hs300_data_5min --region hs300 --interval 5min
    ```
    
### Collector *Baostock high frequency* data to volaris
> collector *Baostock high frequency* data and *dump* into `volaris` format.
> If the above ready-made data can't meet users' requirements,  users can follow this section to crawl the latest data and convert it to volaris-data.
  1. download data to csv: `python scripts/data_collector/baostock_5min/collector.py download_data`
     ...
     - examples:
          ```bash
          # cn 5min data
          python collector.py download_data --source_dir ~/.volaris/stock_data/source/hs300_5min_original --start 2022-01-01 --end 2022-01-30 --interval 5min --region HS300
          ```
  2. normalize data: `python scripts/data_collector/baostock_5min/collector.py normalize_data`
     ...
     - parameters:
          ...
          - `interval`: `5min`
            > if **`interval == 5min`**, `volaris_data_1d_dir` cannot be `None`
          - `region`: `HS300`
          ...
          - `volaris_data_1d_dir`: volaris directory(1d data)
            if interval==5min, volaris_data_1d_dir cannot be None, normalize 5min needs to use 1d data;
            ```
                # volaris_data_1d can be obtained like this:
                python scripts/get_data.py volaris_data --target_dir ~/.volaris/volaris_data/cn_data --interval 1d --region cn --version v3
            ```
      - examples:
        ```bash
        # normalize 5min cn
        python collector.py normalize_data --volaris_data_1d_dir ~/.volaris/volaris_data/cn_data --source_dir ~/.volaris/stock_data/source/hs300_5min_original --normalize_dir ~/.volaris/stock_data/source/hs300_5min_nor --region HS300 --interval 5min
        ```
  3. dump data: `python scripts/dump_bin.py dump_all`
     ...
     - examples:
       ```bash
       # dump 5min cn
       python dump_bin.py dump_all --data_path ~/.volaris/stock_data/source/hs300_5min_nor --volaris_dir ~/.volaris/volaris_data/hs300_5min_bin --freq 5min --exclude_fields date,symbol
       ```