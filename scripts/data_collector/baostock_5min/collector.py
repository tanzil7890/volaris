...
class BaostockNormalizeHS3005min(BaseNormalize):
    ...
    def __init__(
        ...
        volaris_data_1d_dir: str, Path
            the volaris data to be updated for yahoo, usually from: Normalised to 5min using local 1d data
        date_field_name: str
...
class Run(BaseRun):
    ...
    def normalize_data(
        ...
        Attention
        ---------
        volaris_data_1d_dir cannot be None, normalize 5min needs to use 1d data;
        ...
        if volaris_data_1d_dir is None or not Path(volaris_data_1d_dir).expanduser().exists():
            raise ValueError(
                "If normalize 5min, the volaris_data_1d_dir parameter must be set: --volaris_data_1d_dir <user volaris 1d data >, Reference: https://github.com//volaris/tree/main/scripts/data_collector/yahoo#automatic-update-of-daily-frequency-datafrom-yahoo-finance"
            )
