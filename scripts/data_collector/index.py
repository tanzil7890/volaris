...
class IndexBase:
    ...
    def __init__(
        ...
        index_name: str
            index name
        volaris_dir: str
            volaris directory, by default Path(__file__).resolve().parent.joinpath("volaris_data")
        freq: str
        ...
        if volaris_dir is None:
            volaris_dir = Path(__file__).resolve().parent.joinpath("volaris_data")
        self.instruments_dir = Path(volaris_dir).expanduser().resolve().joinpath("instruments")
        self.instruments_dir.mkdir(exist_ok=True, parents=True)
        self.cache_dir = Path(f"~/.cache/volaris/index/{self.index_name}").expanduser().resolve()
    ...
    def save_new_companies(self):
        ...
        Examples
        -------
            $ python collector.py save_new_companies --index_name CSI300 --volaris_dir ~/.volaris/volaris_data/cn_data
        """
    ...
    def parse_instruments(self):
        ...
        Examples
        -------
            $ python collector.py parse_instruments --index_name CSI300 --volaris_dir ~/.volaris/volaris_data/cn_data
        """
