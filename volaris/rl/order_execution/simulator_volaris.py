...
from volaris.backtest import collect_data_loop, get_strategy_executor
from volaris.backtest.decision import BaseTradeDecision, Order, TradeRangeByTime
from volaris.backtest.executor import NestedExecutor
from volaris.rl.data.integration import init_volaris
from volaris.rl.simulator import Simulator
...
class SingleAssetOrderExecution(Simulator[Order, SAOEState, float]):
    """Single-asset order execution (SAOE) simulator which is implemented based on Volaris backtest tools.
    ...
    volaris_config
        Configuration used to initialize Volaris. If it is None, Volaris will not be initialized.
    cash_limit:
        Cash limit.
    """
    ...
    def __init__(
        ...
        strategy_config = {
            ...
            "module_path": "volaris.rl.strategy.single_order",
            "kwargs": {
                "order": order,
        ...
        self.reset(order, strategy_config, executor_config, exchange_config, volaris_config, cash_limit)
    ...
    def reset(
        ...
        if volaris_config is not None:
            init_volaris(volaris_config)
