...
from volaris.backtest import CommonInfrastructure, Order
from volaris.backtest.decision import BaseTradeDecision, TradeDecisionWithDetails, TradeDecisionWO, TradeRange
from volaris.backtest.exchange import Exchange
from volaris.backtest.executor import BaseExecutor
from volaris.backtest.utils import LevelInfrastructure, get_start_end_idx
from volaris.constant import EPS, ONE_MIN, REG_CN
from volaris.rl.data.native import IntradayBacktestData, load_backtest_data
from volaris.rl.interpreter import ActionInterpreter, StateInterpreter
from volaris.rl.order_execution.state import SAOEMetrics, SAOEState
from volaris.rl.order_execution.utils import dataframe_append, price_advantage
from volaris.strategy.base import RLStrategy
from volaris.utils import init_instance_by_config
from volaris.utils.index_data import IndexData
from volaris.utils.time import get_day_min_idx_range
...
class SAOEStrategy(RLStrategy):
    ...
    def _create_volaris_backtest_adapter(
        self,
        order: Order,
        trade_decision: BaseTradeDecision,
    ...
    def reset(self, outer_trade_decision: BaseTradeDecision | None = None, **kwargs: Any) -> None:
        ...
        if outer_trade_decision is not None and not outer_trade_decision.empty():
            ...
            for decision in outer_trade_decision.get_decision():
                ...
                self.adapter_dict[order.key_by_day] = self._create_volaris_backtest_adapter(
