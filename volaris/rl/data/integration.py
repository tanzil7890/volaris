...
"""
TODO: This file is used to integrate NeuTrader with Volaris to run the existing projects.
...
def init_volaris(volaris_config: dict) -> None:
    ...
    ----------
    volaris_config:
        Volaris configuration.

        Example::

            {
                "provider_uri_day": DATA_ROOT_DIR / "volaris_1d",
                "provider_uri_1min": DATA_ROOT_DIR / "volaris_1min",
                "feature_root_dir": DATA_ROOT_DIR / "volaris_handler_stock",
    ...
    for granularity in ["1min", "5min", "day"]:
        if f"provider_uri_{granularity}" in volaris_config:
            provider_uri_map[f"{granularity}"] = _convert_to_path(volaris_config[f"provider_uri_{granularity}"]).as_posix()
    ...
    volaris.init(
        ...
        calendar_provider={
            ...
            "module_path": "volaris.data.data",
            "kwargs": {
                "backend": {
                    "class": "FileCalendarStorage",
                    "module_path": "volaris.data.storage.file_storage",
        ...
        kernels=1,
        redis_port=-1,
        clear_mem_cache=False,  # init_volaris will be called for multiple times. Keep the cache for improving performance
    )
