...
__version__bak = __version__  # This version is backup for VolarisConfig.reset_volaris_version
import logging
import os
import platform
import re
...
def init(default_conf="client", **kwargs):
    ...
    **kwargs :
    ...
    if skip_if_reg and C.registered:
        # if we reinitialize Volaris during running an experiment `R.start`.
    ...
    logger.info("volaris successfully initialized based on %s settings." % default_conf)
...
def init_from_yaml_conf(conf_path, **kwargs):
    ...
    :param conf_path: A path to the volaris config in yml format
    """
...
def get_project_path(config_name="config.yaml", cur_path: Union[Path, str, None] = None) -> Path:
    ...
    - Volaris is a sub folder in project path
    - There is a file named `config.yaml` in volaris.
    ...
    - user want to use a relative config path instead of hard-coding volaris config path in code
...
def auto_init(**kwargs):
    """
    This function will init volaris automatically with following priority
    - Find the project configuration and init volaris
        - The parsing process will be affected by the `conf_type` of the configuration file
    - Init volaris with default config
    ...
    .. code-block:: yaml
        ...
        conf_type: ref
        volaris_cfg: '<shared_yaml_config_path>'    # this could be null reference no config from other files
        # following configs in `volaris_cfg_update` is project=specific
        volaris_cfg_update:
            exp_manager:
    ...
    try:
        ...
        if conf_type == "origin":
            # The type of config is just like original volaris config
            ...
            volaris_conf_path = conf.get("volaris_cfg", None)

            # merge the arguments
            volaris_conf_update = conf.get("volaris_cfg_update", {})
            for k, v in kwargs.items():
                if k in volaris_conf_update:
                    logger.warning(f"`volaris_conf_update` from conf_pp is override by `kwargs` on key '{k}'")
            volaris_conf_update.update(kwargs)

            init_from_yaml_conf(volaris_conf_path, **volaris_conf_update)
