...
from volaris.model.ens.ensemble import RollingEnsemble
from volaris.model.trainer import TrainerR
from volaris.utils import get_cls_kwargs, init_instance_by_config
from volaris.utils.data import update_config
from volaris.workflow import R
from volaris.workflow.record_temp import SignalRecord
from volaris.workflow.task.collect import RecorderCollector
from volaris.workflow.task.gen import RollingGen, task_generator
from volaris.workflow.task.utils import replace_task_handler_with_cache
...
class Rolling:
    ...
    **NOTE**
    ...
    def __init__(
        ...
        # FIXME:
        # - the volaris_init section will be ignored by me.
    ...
    def get_task_list(self) -> List[dict]:
        ...
        for t in task_l:
            ...
            t["record"] = ["volaris.workflow.record_temp.SignalRecord"]
        return task_l
    ...
    def _update_rolling_rec(self):
        ...
        for record in records:
            ...
            r = init_instance_by_config(
                record,
                recorder=rec,
                default_module="volaris.workflow.record_temp",
            )
            r.generate()
