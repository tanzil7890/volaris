...
from volaris.config import C, VolarisConfig
...
def complex_parallel(paral: Parallel, complex_iter):
    ...
    >>> from volaris.utils.paral import complex_parallel
    >>> from joblib import Parallel, delayed
...
class call_in_subproc:
    ...
    def __init__(self, func: Callable, volaris_config: VolarisConfig = None):
        """
        Parameters
        ----------
        func : Callable
        ...
        volaris_config : VolarisConfig
            Volaris config for initialization in subprocess
        ...
        """
        self.func = func
        self.volaris_config = volaris_config

    def _func_mod(self, *args, **kwargs):
        """Modify the initial function by adding Volaris initialization"""
        if self.volaris_config is not None:
            C.register_from_C(self.volaris_config)
