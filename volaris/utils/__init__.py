...
def get_date_by_shift(
    ...
    """
    from volaris.data import D  # pylint: disable=C0415
...
def time_to_slc_point(t: Union[None, str, pd.Timestamp]) -> Union[None, pd.Timestamp]:
    """
    Time slicing in Volaris or Pandas is a frequently-used action.
    ...
    """
    if t is None:
        # None represents unbounded in Volaris or Pandas(e.g. df.loc[slice(None, "20210303")]).
        return t
    else:
...
def exists_volaris_data(volaris_dir):
    volaris_dir = Path(volaris_dir).expanduser()
    if not volaris_dir.exists():
        return False

    calendars_dir = volaris_dir.joinpath("calendars")
    instruments_dir = volaris_dir.joinpath("instruments")
    features_dir = volaris_dir.joinpath("features")
    # check dir
...
def check_volaris_data(volaris_config):
    inst_dir = Path(volaris_config["provider_uri"]).joinpath("instruments")
    for _p in inst_dir.glob("*.txt"):
        assert len(pd.read_csv(_p, sep="\t", nrows=0, header=None).columns) == 3, (
            f"\nThe {str(_p.resolve())} of volaris data is not equal to 3 columns:"
            f"\n\tIf you are using the data provided by volaris: "
            f"https://volaris.readthedocs.io/en/latest/component/data.html#volaris-format-dataset"
            f"\n\tIf you are using your own data, please dump the data again: "
            f"https://volaris.readthedocs.io/en/latest/component/data.html#converting-csv-format-into-volaris-format"
        )
...
#################### Wrapper #####################
class Wrapper:
    """Wrapper class for anything that needs to set up during volaris.init"""
    ...
    def __getattr__(self, key):
        if self.__dict__.get("_provider", None) is None:
            raise AttributeError("Please run volaris.init() first using volaris")
