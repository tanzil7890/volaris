...
class VolarisLogger(metaclass=MetaLogger):
    """
    Customized logger for Volaris.
    """
...
class _VOlarisLoggerManager:
    ...
    def __call__(self, module_name, level: Optional[int] = None) -> VolarisLogger:
        """
        ...
        if not module_name.startswith("volaris."):
            # Add a prefix of volaris. when the requested ``module_name`` doesn't start with ``volaris.``.
            # If the module_name is already volaris.xxx, we do not format here. Otherwise, it will become volaris.volaris.xxx.
            module_name = "volaris.{}".format(module_name)

        # Get logger.
        module_logger = self._loggers.setdefault(module_name, VolarisLogger(module_name))
        module_logger.setLevel(level)
        return module_logger


get_module_logger = _VOlarisLoggerManager()
...
def set_global_logger_level(level: int, return_orig_handler_level: bool = False):
    """set volaris.xxx logger handlers level
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import volaris
            import logging
            from volaris.log import get_module_logger, set_global_logger_level
            volaris.init()
    ...
    """
    _handler_level_map = {}
    volaris_logger = logging.root.manager.loggerDict.get("volaris", None)  # pylint: disable=E1101
    if volaris_logger is not None:
        for _handler in volaris_logger.handlers:
...
def set_global_logger_level_cm(level: int):
    """set volaris.xxx logger handlers level to use contextmanager
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import volaris
            import logging
            from volaris.log import get_module_logger, set_global_logger_level_cm
            volaris.init()
