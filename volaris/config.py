...
from volaris.constant import REG_CN, REG_US, REG_TW
...
class QSettings(BaseSettings):
    """
    Volaris's settings.
    It tries to provide a default settings for most of Volaris's components.
    But it would be a long journey to provide a comprehensive settings for all of Volaris's components.

    Here is some design guidelines:
    - The priority of settings is
        - Actively passed-in settings, like `volaris.init(provider_uri=...)`
        - The default settings
            - QSettings tries to provide default settings for most of Volaris's components.
    """

    mlflow: MLflowSettings = MLflowSettings()
    provider_uri: str = "~/.volaris/volaris_data/cn_data"
...
_default_config = {
    ...
    # config it in volaris.init()
    # "provider_uri" str or dict:
    #   # str
    #   "~/.volaris/stock_data/cn_data"
    #   # dict
    #   {"day": "~/.volaris/stock_data/cn_data", "1min": "~/.volaris/stock_data/cn_data_1min"}
    # NOTE: provider_uri priority:
    #   1. backend_config: backend_obj["kwargs"]["provider_uri"]
    #   2. backend_config: backend_obj["kwargs"]["provider_uri_map"]
    #   3. volaris.init: provider_uri
    "provider_uri": "",
    # cache
    ...
    "redis_task_db": 1,
    "redis_password": None,
    # This value can be reset via volaris.init
    "logging_level": logging.INFO,
    # Global configuration of volaris log
    ...
    "logging_config": {
        ...
        # [1] https://github.com//volaris/pull/1661
        # [2] https://github.com/pytest-dev/pytest/issues/3697
        "loggers": {"volaris": {"level": logging.DEBUG, "handlers": ["console"], "propagate": False}},
        # To let volaris work with other packages, we shouldn't disable existing loggers.
    ...
    "exp_manager": {
        ...
        "module_path": "volaris.workflow.expm",
        "kwargs": {
...
MODE_CONF = {
    "server": {
        ...
        "local_cache_path": Path("~/.cache/volaris_simple_cache").expanduser().resolve(),
        "mount_path": None,
    },
    "client": {
        ...
        # The nfs should be auto-mounted by volaris on other
...
HIGH_FREQ_CONFIG = {
    "provider_uri": "~/.volaris/volaris_data/cn_data_1min",
    "dataset_cache": None,
...
class VolarisConfig(Config):
    ...
    class DataPathManager:
        ...
        def format_provider_uri(provider_uri: Union[str, dict, Path]) -> dict:
            ...
            if isinstance(provider_uri, (str, dict, Path)):
                if not isinstance(provider_uri, dict):
                    provider_uri = {VolarisConfig.DEFAULT_FREQ: provider_uri}
            else:
                raise TypeError(f"provider_uri does not support {type(provider_uri)}")
            for freq, _uri in provider_uri.items():
                if VolarisConfig.DataPathManager.get_uri_type(_uri) == VolarisConfig.LOCAL_URI:
        ...
        def get_uri_type(uri: Union[str, Path]):
            ...
            if is_nfs_or_win and not is_win:
                return VolarisConfig.NFS_URI
            else:
                return VolarisConfig.LOCAL_URI
        ...
        def get_data_uri(self, freq: Optional[Union[str, Freq]] = None) -> Path:
            ...
            if freq is None or freq not in self.provider_uri:
                freq = VolarisConfig.DEFAULT_FREQ
            _provider_uri = self.provider_uri[freq]
            if self.get_uri_type(_provider_uri) == VolarisConfig.LOCAL_URI:
                return Path(_provider_uri)
            elif self.get_uri_type(_provider_uri) == VolarisConfig.NFS_URI:
    ...
    def set(self, default_conf: str = "client", **kwargs):
        """
        configure volaris based on the input parameters
    ...
    def register(self):
        ...
        from .workflow import R, VolarisRecorder  # pylint: disable=C0415
        from .workflow.utils import experiment_exit_handler  # pylint: disable=C0415

        register_all_ops(self)
        register_all_wrappers(self)
        # set up VolarisRecorder
        exp_manager = init_instance_by_config(self["exp_manager"])
        qr = VolarisRecorder(exp_manager)
        R.register(qr)
        # clean up experiment when python program ends
        experiment_exit_handler()

        # Supporting user reset volaris version (useful when user want to connect to volaris server with old version)
        self.reset_volaris_version()

        self._registered = True

    def reset_volaris_version(self):
        import volaris  # pylint: disable=C0415

        reset_version = self.get("volaris_reset_version", None)
        if reset_version is not None:
            volaris.__version__ = reset_version
        else:
            volaris.__version__ = getattr(volaris, "__version__bak")
            # Due to a bug? that converting __version__ to _VolarisConfig__version__bak
...
# global config
C = VolarisConfig(_default_config)
