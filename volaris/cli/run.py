...
import volaris
from volaris.config import C
from volaris.log import get_module_logger
from volaris.model.trainer import task_train
from volaris.utils import set_log_with_config
...
def workflow(config_path, experiment_name="workflow", uri_folder="mlruns"):
    """
    This is a Volaris CLI entrance.
    User can run the whole Quant research workflow defined by a configure file
    - the code is located here ``volaris/cli/run.py``

    User can specify a base_config file in your workflow.yml file by adding "BASE_CONFIG_PATH".
    Volaris will load the configuration in BASE_CONFIG_PATH first, and the user only needs to update the custom fields
    in their own workflow.yml file.

    For examples:

        volaris_init:
            provider_uri: "~/.volaris/volaris_data/cn_data"
            region: cn
    ...
    if "exp_manager" in config.get("volaris_init"):
        volaris.init(**config.get("volaris_init"))
    else:
        exp_manager = C["exp_manager"]
        exp_manager["kwargs"]["uri"] = "file:" + str(Path(os.getcwd()).resolve() / uri_folder)
        volaris.init(**config.get("volaris_init"), exp_manager=exp_manager)
