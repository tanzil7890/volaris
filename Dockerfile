...
RUN conda create --name volaris_env python=3.8 -y
RUN echo "conda activate volaris_env" >> ~/.bashrc
ENV PATH /opt/conda/envs/volaris_env/bin:$PATH
...
RUN if [ "$IS_STABLE" = "yes" ]; then \
        python -m pip install pyvolaris; \
    else \
